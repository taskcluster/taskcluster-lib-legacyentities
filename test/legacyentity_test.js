suite("LegacyEntity", function() {
  ...
  var Item = AbstractItem.configure({
    ...
    tableName:    cfg.get('azureTestTableName') + 'LegacyEntity'
  });
  ...
  /*
  test("Item.deleteTable (ignore if not exists)", function() {
    return Item.createTable().then(function() {
      return Item.deleteTable();
    };
  });*/
  ...
  // Test instanceof
  test("item instanceof Item, AbstractItem, LegacyEntity", function() {
    return Item.create({
      pk:       slugid.v4(),
      rk:       "row-key",
      str:      "Hello World",
